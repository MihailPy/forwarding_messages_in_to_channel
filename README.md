...
Этот проект представляет собой Telegram-бота, который пересылает
сообщения из указанных источников (чатов или каналов) в целевые каналы,
используя две разные учетные записи Telegram. Проект основан на библиотеке
Telethon и поддерживает параллельную работу нескольких аккаунтов.
...
1. Настройте фильтры и обработчики в `config.py`:
